...
# Helper to read the proper stat value
func _get_stat_value(stats: PlayerStats, stat_name: String) -> int:
	match stat_name:
		"Attack":
			return stats.get_player_attack()
		"Defense":
			return stats.get_player_defense()
		"Speed":
			return stats.get_player_speed()
		_:
			return 0
		...
		EffectType.RDM:
			...
			match which:
				...
				2:
					...
					stat_name = "Speed"
			var new_value := _get_stat_value(stats, stat_name)
			if damage > 0:
				return "You lost %d HP but %s increased by %d!\nPlayer %s changed to: %d" % \
					[damage, stat_name, boost, stat_name, new_value]
			else:
				return "%s increased by %d! (You were already at 1 HP.)\nPlayer %s changed to: %d" % \
					[stat_name, boost, stat_name, new_value]
		_:
			return ""
